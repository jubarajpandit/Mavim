...
{
    public class CreateChildTopicCommandTest
    {
        ...
        public async Task Execute_ValidArguments_OkObjectResult()
        {
            ...
            mavimDataModelMock.Setup(model => model.Factories.CommandFactory.CreateCreateElementLowerCommand(It.IsAny<IElement>(), It.IsAny<string>())).Returns(modelCommandMock.Object);
            ...
            var command = new CreateChildTopicCommand(dataAccessMock.Object, dataLanguage.Object);
        ...
        {
            // Arrange
            var exceptionMessage = "Supplied reference topic to create child topic not found: d5926266c2v0";
            var topicName = "test";
            var topicIcon = "test";
            var mavimDataModelMock = new Mock<IMavimDatabaseModel>();
            mavimDataModelMock.Setup(model => model.ElementRepository.GetElement(It.IsAny<IDcvId>())).Returns((IElement)null);
        ...
        {
            // Arrange
            var exceptionMessage = "Invalid topic icon supplied for this topic: MvmSRV_ELEtpe_mav";
            var topicName = "test";
            var topicMock = new Mock<IElement>();
            var elementTypeMock = new Mock<IElementType>();
            elementTypeMock.Setup(mock => mock.Type).Returns(Server.ELEBuffer.MvmSrv_ELEtpe.MvmSRV_ELEtpe_mav);
        ...
        {
            // Arrange
            var exceptionMessage = "Cannot create child topic in given topic";
            var topicName = "test";
            var topicMock = new Mock<IElement>();
            var elementTypeMock = new Mock<IElementType>();
            elementTypeMock.Setup(mock => mock.Type).Returns(Server.ELEBuffer.MvmSrv_ELEtpe.MvmSRV_ELEtpe_mav);
