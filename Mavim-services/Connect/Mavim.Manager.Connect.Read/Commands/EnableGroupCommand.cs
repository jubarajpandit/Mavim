...
{
    public static class EnableGroupCommand
    {
        // Command
        ...
        public class Handler : BaseHandler, IRequestHandler<Command, Unit>
        {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (!group.Disabled)
                    throw new ConflictException($"Group with id: {groupId} is already enabled");

                var groupValue = Map<GroupValue>(group.Value);
                foreach (Guid userId in groupValue?.Users)
                    await AddGroupToUser(groupId, userId, cancellationToken);
            ...
            private async Task AddGroupToUser(Guid groupId, Guid userId, CancellationToken cancellationToken)
            {
                var user = await _dbContext.Users.AsNoTracking().FirstOrDefaultAsync(x => x.Id == userId, cancellationToken);
                if (user is null) throw new ConflictException(string.Format(Logging.USER_NOT_FOUND, userId));
