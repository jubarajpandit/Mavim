...
{
    public static class DeleteUsersFromGroupCommand
    {
        // Command
        ...
        public class Handler : BaseHandler, IRequestHandler<Command, Unit>
        {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                foreach (Guid userId in userIds)
                    await UpdateUser(userId, groupId, cancellationToken);

                RemoveUsersFromGroup(group, userIds, modelVersion, aggregateId);
            ...
            private void RemoveUsersFromGroup(GroupTable group, IEnumerable<Guid> userIds, int modelVersion, int aggregateId)
            {
                var groupValue = Map<GroupValue>(group.Value);
                var updatedUsers = groupValue.Users.Except(userIds).ToList();
            ...
            private async Task UpdateUser(Guid userId, Guid groupId, CancellationToken cancellationToken)
            {
                var user = await _dbContext.Users.AsNoTracking().FirstOrDefaultAsync(x => x.Id == userId, cancellationToken);
                if (user is null) throw new UnprocessableEntityException(string.Format(Logging.USER_NOT_FOUND, userId));

                var userValue = Map<UserValue>(user.Value);
                var groups = userValue.Groups.ToList();
