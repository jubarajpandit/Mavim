...
namespace Mavim.Manager.Connect.Read.Test.Commands
{
    public class DisableUserCommandTest
    {
        ...
        public async Task DisableUserCommand_ValidArguments_GroupDisabledAndUsersUpdated()
        {
            // Arrange
            ...
            var handler = new DisableUserCommand.Handler(dbContext);
            var request = new DisableUserCommand.Command(userId, modelVersion, aggregateId + 1);
            ...
            Assert.True(disabledUser.Disabled);
        }

        [Fact]
        [Trait("category", "connectRead")]
        public async Task DisableUserCommand_RequestNull_ArgumentNullException()
        {
            // Arrange
        ...
        [Fact]
        [Trait("category", "connectRead")]
        public async Task DisableUserCommand_LowerAggregateId_UnprocessableEntityException()
        {
            // Arrange
            var dbContext = GetMockContext();
            var groupId = Guid.NewGuid();
            ...
            var handler = new DisableUserCommand.Handler(dbContext);
            var request = new DisableUserCommand.Command(userId, modelVersion, aggregateId);
            var cancellationToken = new System.Threading.CancellationToken();
        ...
        [Fact]
        [Trait("category", "connectRead")]
        public async Task DisableUserCommand_HigherAggregateId_UnprocessableEntityException()
        {
            // Arrange
            var dbContext = GetMockContext();
            var groupId = Guid.NewGuid();
            ...
            var handler = new DisableUserCommand.Handler(dbContext);
            var request = new DisableUserCommand.Command(userId, modelVersion, higherAggregateId);
            var cancellationToken = new System.Threading.CancellationToken();
