...
{
    public class EnableGroupCommandTest
    {
        [Fact]
        [Trait("category", "connectRead")]
        public async Task EnableGroupCommand_ValidArguments_GroupDisabledAndUsersUpdated()
        {
            // Arrange
            ...
            var userObject = new UserTable(userId, GetUserValue(userId, companyId, Guid.NewGuid()), modelVersion, aggregateId, companyId, enabled, lastUpdated);
            ...
            var handler = new EnableGroupCommand.Handler(dbContext);
            var request = new EnableGroupCommand.Command(groupId, modelVersion, aggregateId + 1);
            ...
            Assert.False(disabledGroup.Disabled);
            ...
            Assert.Contains(updatedUserValue.Groups, x => x == groupId);
        }

        [Fact]
        [Trait("category", "connectRead")]
        public async Task EnableGroupCommand_RequestNull_ArgumentNullException()
        {
            // Arrange
        ...
        [Fact]
        [Trait("category", "connectRead")]
        public async Task EnableGroupCommand_GroupAlreadyEnabled_ConflictException()
        {
            // Arrange
            var dbContext = GetMockContext();
            var groupId = Guid.NewGuid();
            ...
            dbContext.Groups.Add(group);
            dbContext.SaveChanges();
            var expectedMessage = $"Group with id: {groupId} is already enabled";
            var handler = new EnableGroupCommand.Handler(dbContext);
            var request = new EnableGroupCommand.Command(groupId, modelVersion, aggregateId + 1);
            var cancellationToken = new System.Threading.CancellationToken();
        ...
        [Fact]
        [Trait("category", "connectRead")]
        public async Task EnableGroupCommand_LowerAggregateId_UnprocessableEntityException()
        {
            // Arrange
            var dbContext = GetMockContext();
            var groupId = Guid.NewGuid();
            ...
            var handler = new EnableGroupCommand.Handler(dbContext);
            var request = new EnableGroupCommand.Command(groupId, modelVersion, aggregateId);
            var cancellationToken = new System.Threading.CancellationToken();
        ...
        [Fact]
        [Trait("category", "connectRead")]
        public async Task EnableGroupCommand_HigherAggregateId_UnprocessableEntityException()
        {
            // Arrange
            var dbContext = GetMockContext();
            var groupId = Guid.NewGuid();
            ...
            var handler = new EnableGroupCommand.Handler(dbContext);
            var request = new EnableGroupCommand.Command(groupId, modelVersion, higherAggregateId);
            var cancellationToken = new System.Threading.CancellationToken();
        ...
        [Fact]
        [Trait("category", "connectRead")]
        public async Task EnableGroupCommand_UserInGroupDoesNotExist_ConflictException()
        {
            // Arrange
            var dbContext = GetMockContext();
            var groupId = Guid.NewGuid();
